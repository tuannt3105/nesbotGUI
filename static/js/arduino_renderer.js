...
const myarduino_data = './arduino-cli';
const mysketchbook_path = './arduino-cli/sketches';
...
*/
//console.log('getValueView : ');
//console.log(cli.getValueView());
...
$('#uploader_flash').on('click', function() {
  ...
  sketchName = 'sketch010102';
  console.log($('#uploader_boards option:selected').data());
  //fqbn = 'arduino:avr:nano:cpu=atmega328';//$('#uploader_boards option:selected').data('fqbn');
  //port = 'COM4';//$('#uploader_boards option:selected').data('port');
  fqbn = $('#uploader_boards option:selected').data('fqbn');
  port = $('#uploader_ports option:selected').data('port');

console.log('Port: '+port+' - fqbn: '+fqbn);
  // create sketch
  ...
  cli.createSketch(sketchName).then(function(sketchPath) {
    ...
    cli.newcompile(fqbn, sketchName).then(function(result) {
        console.log('Compiled successfuly');
        console.log(result)
        log('Compiled successfuly');
        // upload sketch
        cli.newupload(port, fqbn, sketchName).then(function(result) {
